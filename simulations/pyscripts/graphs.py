...
"""
module launch - Submits an execution to pbs queue.

This module launches a job
"""
...
def time2d(data, graph_title, output_file_bar, output_file_scatter,
    ...
    dfSuccess = pd.DataFrame(data["success"],
                             columns=['user', 'subTime', 'type', 'totalWaitTime'])
...
def cpuTime2d(data, graph_title, output_file, extensions=["eps"]):
    ...
    step = dfCpuTime["numCore"].max() // 8
    tick = step
    ticks = []
    for t in range(8):
        ticks.append(tick-1)
        tick = tick + step
    loc = plticker.FixedLocator(ticks)  # ticks at regular intervals
    #loc = plticker.MultipleLocator(base=step+1)  # ticks at regular intervals
    # loc = plticker.AutoLocator()
    ax.xaxis.set_major_locator(loc)
    ax.set_xlim(left=0)
...
def cost3d(df_table, graph_title, output_file, min_key, max_key,
           extensions=["eps"]):
    """
...
def catplot2d(df_table, graph_title, output_file, extensions=["png"]):
...
def catplotp2d(df_table, current_row, x_coord,graph_title, output_file, extensions=["png"], column=False):
    ...
    # g.ax.set_title(graph_title)
    g.set_xticklabels(rotation=30, ha='right')
    g.set_xlabels("Physical machines")
