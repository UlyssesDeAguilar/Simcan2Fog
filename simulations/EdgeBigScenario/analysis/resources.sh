...
opp_scavetool export -f 'module=~"**.fg*.resourceManager" AND name=~"allocated*:vector" AND runattr:configname=~targetFog' -F CSV-R -o targetFogRes1.csv $RDIR/*.vec

opp_scavetool export -f 'module=~"**.dc.resourceManager" AND name=~"allocated*:vector" AND runattr:configname=~targetFog' -F CSV-R -o targetFogRes2.csv $RDIR/*.vec
