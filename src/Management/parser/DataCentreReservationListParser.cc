...
#include "DataCentreReservationListParser.h"

DataCentreReservationListParser::DataCentreReservationListParser(const char *s) : DataCentreListParser(s) {
    // TODO Auto-generated constructor stub

}

DataCentreReservationListParser::~DataCentreReservationListParser() {
    // TODO Auto-generated destructor stub
}

int DataCentreReservationListParser::parse() {
        cStringTokenizer tokenizer(parseString);

        // First token, number of data-centres
        if (tokenizer.hasMoreTokens()){
            const char *numDataCentresChr = tokenizer.nextToken();
            numDataCentres = atoi(numDataCentresChr);
        }
        else{
            EV_ERROR << "Cannot read the first token (number of Data-Centres)" << endl;
            result = SC_ERROR;
        }

        // While there are unprocessed items...
        while ((tokenizer.hasMoreTokens()) && (currentDataCentre < numDataCentres) && (result==SC_OK)){
            ...
            if ((tokenizer.hasMoreTokens()) && (result==SC_OK)){
              ...
              EV_ERROR << "Cannot read name of Data-Centre:" << currentDataCentre << endl;
              result = SC_ERROR;
            }
            ...
            if ((tokenizer.hasMoreTokens()) && (result==SC_OK)){
              ...
              EV_ERROR << "Cannot read number of reserved nodes of the Data-Centre:" << currentDataCentre << endl;
              result = SC_ERROR;
            }
            ...
            if (result==SC_OK){
                ...
                dataCentreReservationObj = new DataCentreReservation (dataCentreNameStr, nReservedNodes);
                ...
                while ((tokenizer.hasMoreTokens()) && (currentRack < numComputingRacks) && (result==SC_OK)){
                    ...
                    // Generate Racks and include them in the Data-Centre object!
                        ...
                        for (currentRackInstance = 0; currentRackInstance < numRackInstances; currentRackInstance++){
                            ...
                            dataCentreReservationObj->addRack(rack, false);
                        }
                ...
                while ((tokenizer.hasMoreTokens()) && (currentRack < numStorageRacks) && (result==SC_OK)){
                    ...
                    // Generate Racks and include them in the Data-Centre object!
                        ...
                        for (currentRackInstance = 0; currentRackInstance < numRackInstances; currentRackInstance++){
                            ...
                            // Add current rack to the data-centre
                            dataCentreReservationObj->addRack(rack, true);
                        }
            ...
            parseResult.push_back(dataCentreReservationObj);

            // Process next data-Centre
            currentDataCentre++;
        }
