...
#include "CloudUserInstanceTrace.h"
 ...
 : CloudUserInstancePriority(nullptr, totalUserInstance, nCurrentNumber, nUserInstance, nTotalInstances)
 {
     ...
     // Generate userID
     osStream << "(-1)" << "UserTrace" << "[" << (nUserInstance+1) << "/" << nTotalInstances << "]";
     userID = osStream.str();
     type = "UserTrace";
     ...
     newAppCollection = new AppInstanceCollection (ptrApp, this->userID, 1);
     applications.push_back(newAppCollection);
     appInstances.push_back(new AppInstance("AppCPUIntensive",0,1,getUserID()));
