...
// For reading from the environement variables
#include <cstdlib>
...
protected:
    ...
    virtual void handleMessage(omnetpp::cMessage *msg) override { delete msg; error("This module doesn't receive messages"); }

public:
    virtual std::unique_ptr<Application> searchApp(const std::string& name) override;
    virtual std::unique_ptr<VirtualMachine> searchVirtualMachine(const std::string& name) override;
    virtual std::unique_ptr<Sla> searchSLA(const std::string& name) override;
    virtual std::unique_ptr<Sla::VMCost> searchVMCost(const std::string& sla, const std::string& vmType) override;
};
